...
/**
 * Class encapsulating a single File (or series of files) Default is one file
 * series per xy posititon
 */
class FileSet {
	private static final int SPACE_FOR_PARTIAL_OME_MD = 2000; // this should be
																// more than
																// enough
...
//	private Studio studio_;
	...
	public FileSet(JSONObject firstImageTags, StorageMultipageTiff masterStorage, OMEMetadata omeMetadata,
...
//		this.settings = studio_.acquisitions().getAcquisitionSettings();
		...
		if (MMStudio.USE_CUSTOM_PATH) {
			...
			baseFilename_ = MMStudio.CUSTOM_FILE_NAME;
	...
	public void writeImage(TaggedImage img) throws IOException {
		...
		try {
			...
			// reset in case acquisitin order is position then time and all
			// files not split by position
			currentFrame_ = frame;
	...
	private void writeToMetadataFile(JSONObject md) throws JSONException {
		try {
			mdWriter_.write(",\n\"FrameKey-" + MDUtils.getFrameIndex(md) + "-" + MDUtils.getChannelIndex(md) + "-"
	...
	private void startMetadataFile() throws JSONException {
		metadataFileFullPath_ = masterStorage_.getDiskLocation() + "/" + baseFilename_ + "_metadata.txt";
		try {
	...
	private String createBaseFilename(JSONObject firstImageTags) {
		...
		if (MMStudio.USE_CUSTOM_PATH)
			baseFilename = MMStudio.CUSTOM_FILE_NAME;
		else {
	...
	public void finishAbortedAcqIfNeeded() {
		if (expectedImageOrder_ && splitByXYPosition_ && !masterStorage_.timeFirst()) {
			try {
				// One position may be on the next frame compared to others.
	...
	void checkForExpectedImageOrder(JSONObject tags) {
		try {
			...
			if (slice != nextExpectedSlice_ || channel != nextExpectedChannel_ || frame != nextExpectedFrame_) {
				expectedImageOrder_ = false;
			}
