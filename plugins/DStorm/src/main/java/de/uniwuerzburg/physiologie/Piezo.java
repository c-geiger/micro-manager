...
public class Piezo {
	...
	public double retrieveZPos() {
		...
		try {
			...
			System.out.println("can't obtain zPosition: " + tempPoszS);
			e.printStackTrace();
		}
	...
	public int retrieveOutputcycleID() {
		...
		try {
			...
			System.out.println("problem getting OCID :" + answer);
			e.printStackTrace();
		}
	...
	// Set Configuration of trigger input Type Triggerinputchannel 1 parametre
	// to : String "edge" or "level" triggered
	...
	// Get Configuration of trigger input Type Triggerinputchannel 1 : "edge" or
	// "level" triggered
	...
	// Set Configuration of trigger input Polarity Triggerinputchannel 1
	// parametre to : "high" =active high or "low" =active low
	...
	// Get Configuration of trigger input Polarity Triggerinputchannel 1 :
	...
	// set Piezo rectangular positiv TriggerOutput to a fraction of
	// 1/wavepointfraction
	public void setPiezoTriggerOutput(int wavepointfraction, int segmentLength) {
		...
		try {
			...
			// int
			// wavePoints=Integer.parseInt(mmc.getSerialPortAnswer("Port","\n").substring(4));
	...
	public void setPiezoWavetableParametres(String direction, int wavetableID, int segmentLength,
			double cycleDistance) {
		try {
			if (direction.equals("upwards")) {
				mmc.setSerialPortCommand("Port",
						"WAV " + String.valueOf(wavetableID) + " X LIN " + String.valueOf(segmentLength) + " -"
								+ String.valueOf(cycleDistance) + " 0 " + String.valueOf(segmentLength) + " 0 0 ",
						"\n");
			}
	...
	public int initializePiezoScan(String direction) {
		...
		if (direction.equals("calibration")) {
			startPos = accSettings.startPositionCalibration;
		}
	...
	public void InitializePiezoDevice(String com) {
		...
		try {
			mmc.loadDevice("Port", "SerialManager", com);
			props = mmc.getDevicePropertyNames("Port");
			for(String prop : props){
				System.out.println(prop);
			}
			mmc.setProperty("Port", "AnswerTimeout", "2000.0000");
			System.out.println("AnswerTimeout: " + mmc.getProperty("Port", "AnswerTimeout"));
		} catch (Exception ex) {
			System.out.println("load portdevice error ," + ex);
		}
